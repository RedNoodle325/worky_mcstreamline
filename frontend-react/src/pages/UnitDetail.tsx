    ...
    <div>
      ...
      <div className="page-header" style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <Link to={backHref} className="btn btn-secondary btn-sm">
            ← {site ? site.name : 'Back'}
          </Link>
          <div>
            ...
            <div className="page-subtitle" style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              {site && (
                <span style={{ color: 'var(--text3)' }}>
                  @{' '}
                  <Link to={`/sites/${site.id}`} style={{ color: 'var(--text3)' }}>{site.name}</Link>
                </span>
              )}
            </div>
          </div>
        </div>
        <Link to={`/units/${id}/edit`} className="btn btn-secondary btn-sm">Edit Unit</Link>
      </div>
